...
import { ICinematicPropertyGroupTrack } from "../../../editor/cinematic/track";
...
export class CinematicPropertyGroupTrack {
     ...
     * Constructor.
     */
    public constructor(public track: ICinematicPropertyGroupTrack, public tracks: Nullable<Tracks>) {
        // Empty for now...
    }
}
...
export class CinematicPropertyGroupTrackInspector extends AbstractInspector<CinematicPropertyGroupTrack, ICinematicPropertyTrackInspectorState> {
    ...
    public renderContent(): React.ReactNode {
        const nodes = this.selectedObject.track.nodeIds.map((_, index) => {
            return this._getNodeIdField(index);
        });

        nodes.push(<InspectorButton label="Add..." onClick={() => this._handleAddNode()} />);
    ...
    private _handleAddNode(): void {
        this.selectedObject.track.nodeIds.push(undefined!);
        this.forceUpdate();
    }
    ...
    private _getNodeIdField(index: number): React.ReactNode {
        const o = { nodeId: this.selectedObject.track.nodeIds[index] ?? undefined };
        ...
        return (
            <InspectorList key={Tools.RandomId()} object={o} property="nodeId" label="Node" noUndoRedo borderLeftColor="forestgreen" dndHandledTypes={["graph/node"]} items={() => {
                ...
                if (v) {
                    this.selectedObject.track.nodeIds[index] = v;
                } else {
                    this.selectedObject.track.nodeIds.splice(index, 1);
                }
