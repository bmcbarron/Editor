...
import { Button, ContextMenu, H4, Menu, MenuDivider, MenuItem, Tree, TreeNodeInfo } from "@blueprintjs/core";
...
export class Tracks extends React.Component<ITracksProps, ITracksState> {
	...
	public render(): React.ReactNode {
		return (
			<div style={{ width: "100%", height: "100%", overflow: "hidden" }}>
				<H4 style={{ lineHeight: "45px", margin: "auto", textAlign: "center", color: "grey" }}>Tracks</H4>
				<div
	...
	public getTracksNodes(): TreeNodeInfo<ITrackOptions>[] {
		return [this._getCameraContent(), ...this._getTracksContent(), this._getAddButton()];
	}
	...
	private _handleNodeClicked(node: TreeNodeInfo<ITrackOptions>): void {
		...
		if (node.nodeData?.track?.property) {
			this.props.editor.inspector.setSelectedObject(new CinematicPropertyTrack(node.nodeData.track.property, this.props.cinematicEditor._tracks));
		}

		if (node.nodeData?.track?.propertyGroup) {
			this.props.editor.inspector.setSelectedObject(new CinematicPropertyGroupTrack(node.nodeData.track.propertyGroup, this.props.cinematicEditor._tracks));
		}
	...
	private _getCameraContent(): TreeNodeInfo<ITrackOptions> {
		return {
			...
			childNodes: [{
				...
				secondaryLabel: <span style={{ color: "grey" }}>(DOF Focus Distance)</span>,
			}, {
				label: "F Stop",
				id: "cinematic-editor-camera-fstop",
				secondaryLabel: <span style={{ color: "grey" }}>(DOF F Stop)</span>,
			}, {
				label: "Focal Length",
				id: "cinematic-editor-camera-focal-length",
				secondaryLabel: <span style={{ color: "grey" }}>(DOF Focal Length)</span>,
			}],
		};
	}
	 ...
	 */
	private _getTracksContent(): TreeNodeInfo<ITrackOptions>[] {
		...
		return this.props.cinematic.tracks.map((t, index) => {
			...
			switch (t.type) {
				case CinematicTrackType.Property:
					const listLabel = (
						<span>
							Node <span style={{ color: "grey" }}>({t.property!.propertyPath})</span>
						</span>
					)
					label = <InspectorList object={t.property} property="nodeId" label={listLabel} noUndoRedo borderLeftColor="forestgreen" dndHandledTypes={["graph/node"]} items={() => {
				...
				case CinematicTrackType.PropertyGroup:
					...
					secondaryLabel = <span style={{ color: "grey" }}>({t.propertyGroup!.propertyPath})</span>;
					break;

				case CinematicTrackType.AnimationGroup:
					label = <InspectorList object={t.animationGroup} property="name" label="Group" noUndoRedo borderLeftColor="darkorange" items={() => {
	 ...
	 */
	private _getAddButton(): TreeNodeInfo<ITrackOptions> {
		return {
			id: "cinematic-editor-add-track-button",
			label: <Button small fill onClick={(ev) => this._handleAddButtonClicked(ev)}>Add...</Button>,
		};
	}
